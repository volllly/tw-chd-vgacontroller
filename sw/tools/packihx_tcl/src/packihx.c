 ...
 * packihx.c:
 *
 * Version 2017/02/10: Usage modifed to: packihx ihxfile hexfile
 *                     Moreover, output to stderr removed (in case of no error)
 *                       in order to enable usage in a TCL environment 
...
/* Globals: input file (IHX format) & output file (HEX format) */
FILE *inFile;
FILE *outFile;
		...
		const Uint8 *data)
{
    ...
    if (fprintf(outFile, ":%02X%04X%02X", len, offset, type) == EOF)
    {
        return -1;
    }
    ...
    for (i = 0; i < len; i++)
    {
        if (fprintf(outFile, "%02X", data[i]) == EOF)
        {
            return -1;
        }
    }
    
    if (fprintf(outFile, "%02X\n", lineChecksum(len, offset, type, data)) == EOF)
    {
    	return -1;
    }
    outlineno++;
...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, "packihx: wrong usage (must be: packihx ihxfile hexfile)\n");
        return -1;
    }
    ...
    if (!rc)
    {
        printf("packihx: read %d lines, wrote %d: Success!\n", lineno, outlineno);
    }
